...
const getCurrent = asyncHandler(async(req, res) => {
    const {_id} = req.user
    const user = await User.findById(_id).select('-refresh -password -role')
    return res.status(200).json({
...
const getUsers = asyncHandler(async(req, res) => {
    const response = await User.find().select('-refresh -password -role')
    return res.status(200).json({
        sucess: response ? true : false,
        user: response
    })
...
const updateUser = asyncHandler(async(req, res) => {
    ...
    const response = await User.findByIdAndUpdate(_id, req.body, {new: true}).select('-pasword -role')
    return res.status(200).json({
        sucess: response ? true : false,
...
const updateUserByAdmin = asyncHandler(async(req, res) => {
    ...
    const response = await User.findByIdAndUpdate(uid, req.body, {new: true}).select('-pasword -role -refreshToken')
    return res.status(200).json({
        sucess: response ? true : false,
        updateUser: response ? response : 'Some thing went wrong'
    })
